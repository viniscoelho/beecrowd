...
{
    for ( int i = 0; i < e.pawns.size(); i++ ) e.matrix[e.pawns[i].first][e.pawns[i].second] = '.';
    for ( int i = 0; i < e.pawns.size(); i++ )
    {
        ii a = e.pawns[i];
        e.pawns[i].first++;
        if ( e.pawns[i].first > 7 ) return true; //couldn't capture this one
        if ( e.matrix[a.first+1][a.second] == 'h' ) return true; //just got hit by a pawn!
...
int bfs(int x, int y, vector<ii>& pawns)
{
    ...
    while ( !b.empty() )
    {
        aux = b.front();
        b.pop();
        if ( !horseMove && movePawns(aux) ) continue;
        horseMove = false;
        for ( int i = 0; i < 8; i++ )
        {
            //original state
            ...
            int p1 = e.x + dir[i][0];
            int p2 = e.y + dir[i][1];
            if ( p1 < 0 || p2 < 0 || p1 > 7 || p2 > 7 ) continue;
            if ( e.matrix[p1][p2] == 'p' )
            {
                e.matrix[e.x][e.y] = '.';
                capture(mp(p1, p2), e);
            ...
            if ( !e.pawns.size() ) return e.count; //captured all of them
