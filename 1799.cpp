...
int main(){
    ...
    for ( int i = 0; i < a; i++ )
    {
        cin >> from >> to;
        int de, para;
        if ( links.count(from) ) de = links[from];
        else
        {
            de = c;
            links[from] = c++;
        }
        if ( from == "Entrada" ) entrada = de;
        else if ( from == "Saida" ) saida = de;
        else if ( from == "*" ) queijo = de;

        if ( links.count(to) ) para = links[to];
        else
        {
            para = c;
            links[to] = c++;
        }
        if ( to == "Entrada" ) entrada = para;
        else if ( to == "Saida" ) saida = para;
        else if ( to == "*" ) queijo = para;
        //cout << de << " " << para << endl;
