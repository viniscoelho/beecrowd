...
int main (){
    ...
    while ( t-- ){
		...
		for ( int i = 0; i < strlen(ch); i++ ){
            if ( ch[i] == '(' ){
                if ( input.length() && isdigit (input[input.length()-1]) ) input += '#';
            }
            else if ( isdigit(ch[i]) ){
				if ( input.length() && input [input.length () - 1] == ')') input += '#';
            }
			input += ch[i];
        }

        for ( size_t i = 0; i < input.length (); i++ ) {
            if ( isdigit(input[i]) || isalpha(input[i]) ) output.push_back(input[i]);
