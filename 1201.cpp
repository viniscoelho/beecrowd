...
void ArvoreBinaria::EmOrdem(No* inicio)
{
    if (inicio != NULL) {
        ...
        if (!qtd && inicio->valor != -1)
            cout << inicio->valor;
        else if (qtd && inicio->valor != -1)
            cout << " " << inicio->valor;
        if (inicio->valor != -1)
            qtd++;
...
{
    if (delnode < current->valor)
        Remove(current->esquerdo, current, delnode);
    else if (delnode > current->valor)
        Remove(current->direito, current, delnode);
    else if (delnode == current->valor) {
        if (current->esquerdo == NULL) {
            if (parent->valor > current->valor)
            ...
            if (parent->valor > current->valor)
                ...
                parent->direito = current->esquerdo;
        } else {
            No* temp;
            int flag = 0;
            parent = current;
