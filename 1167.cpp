...
int main(){
	...
	while ( scanf("%d", &n) && n ){
		...
		else summer[0].prev = summer[0].next = 0;
		for ( i = 1; i < n-1; ++i ){
			scanf(" %s %d", &summer[i].nome, &summer[i].num);
			summer[i].prev = i-1;
			summer[i].next = i+1;
		}
		if ( n > 1 ) scanf(" %s %d", &summer[n-1].nome, &summer[n-1].num);
		int a = summer[0].num, c = 0, qtd = n;
		while ( qtd != 1 ){
			if ( !(a & 1) ) for ( i = 0; i < a; ++i ) c = summer[c].prev;
			else for ( i = 0; i < a; ++i ) c = summer[c].next;
			a = summer[c].num;
